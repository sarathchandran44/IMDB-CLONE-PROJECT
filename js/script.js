...
// (function () {
  ...
  const favButton = document.getElementById("imag1");
  const minButton =document.getElementById("minimze");
  ...
  // minmize button to close favorite section
  function minMize(){
    document.getElementById("favorites-section").style.display = "none";
    favButton.disabled = false;
  };
  favButton.addEventListener("click",Display);
  minButton.addEventListener("click",minMize);
  ...
  async function fetchMovies(search) {
    ...
    const url = `https://www.omdbapi.com/?t=${search}&apikey=f9d55ac1`;
    try {
